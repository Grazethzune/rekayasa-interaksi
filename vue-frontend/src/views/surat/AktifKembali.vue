<template>
  <div class="page-inner">
    <h1 class="title">Aktif Kembali</h1>
    ...
    <section class="card">
      <div class="table-wrapper">
        ...
        <table class="table">
          <thead>
            <tr><th>No</th><th>Tahun Ajar</th><th>Semester Ajar</th></tr>
          </thead>
          <tbody>
...
const filtered = computed(() => {
  ...
  const list = tabel.value.length ? tabel.value : defaultAktif;
  if (!q) return list;
  return list.filter(
    (i) =>
...
const fetchAktif = async () => {
  try {
    ...
    tabel.value = aktif.length ? aktif : defaultAktif;
  } catch (e) {
    tabel.value = defaultAktif;
  }
};
